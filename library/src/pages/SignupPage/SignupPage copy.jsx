...
function SignupPage(props) {
        ...
        if(!!e.target.value) {
            setMessageGroup(messageGroup => {
                return {
                    ...messageGroup,
                    checkPassword: {
                        type: checkPassword === password ? "sucess" : "error",
    ...
    const handleSignupSubmit = () => {
        ...
        signupRequest(signupDate);
    }
    const signupRequest = async (signupDate) => {
        try{
            const response = await axios.post("http://localhost:8080/auth/signup", signupDate);
            if(response.data) {
